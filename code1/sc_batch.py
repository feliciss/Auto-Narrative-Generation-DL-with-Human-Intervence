...
class Sc_Example(object):
  ...
  def __init__(self, text, label, pos_tagging, compression, dependency,  vocab, hps, reward = 1):
      ...
      text_sen_words = text
      if len(text_sen_words) > hps.sc_max_enc_seq_len.value:   #by Mozi: TypeError: unorderable types: int() > Flag(). Hence I add an int()
      ...
      if len(pos_words) > hps.sc_max_enc_seq_len.value:   #by Mozi: TypeError: unorderable types: int() > Flag(). Hence I add an int()
      ...
      if len(dependency_words) > hps.sc_max_enc_seq_len.value:    #by Mozi: TypeError: unorderable types: int() > Flag(). Hence I add an int()
      ...
      if len(label) > hps.sc_max_dec_seq_len.value:   #by Mozi: TypeError: unorderable types: int() > Flag(). Hence I add an int()
  ...
  def get_dec_inp_targ_seqs(self, sequence, max_len, start_id, stop_id):
        ...
        inps = [start_id] + inps[:]
        if len(inps) > max_len.value:
            inps = inps[:max_len.value]

        if len(targets) >= max_len.value:
            targets = targets[:max_len.value - 1]  # no end_token
  ...
  def pad_decoder_inp_targ(self, max_sen_len):
        ...
        while len(self.dec_input) < max_sen_len.value:
            self.dec_input.append(2)



        while len(self.target) < max_sen_len.value:
  ...
  def pad_encoder_inp_targ(self, max_sen_len, pad_doc_id):
      ...
      while len(self.enc_input_text) < max_sen_len.value:
          self.enc_input_text.append(pad_doc_id)


      while len(self.enc_input_pos) < max_sen_len.value:
          self.enc_input_pos.append(pad_doc_id)

      while len(self.enc_input_dep) < max_sen_len.value:
