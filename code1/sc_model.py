...
class Sc_Generator(object):
  ...
  def _add_encoder(self, encoder_inputs, seq_len):
      with tf.variable_scope('encoder'):
          ...
          cell_fw1 = tf.contrib.rnn.LSTMCell(self._hps.hidden_dim.value, initializer=self.rand_unif_init, state_is_tuple=True)
          cell_fw2 = tf.contrib.rnn.LSTMCell(self._hps.hidden_dim.value, initializer=self.rand_unif_init, state_is_tuple=True)

          cell_bw1 = tf.contrib.rnn.LSTMCell(self._hps.hidden_dim.value, initializer=self.rand_unif_init, state_is_tuple=True)
          cell_bw2 = tf.contrib.rnn.LSTMCell(self._hps.hidden_dim.value, initializer=self.rand_unif_init, state_is_tuple=True)
  ...
  def _build_model(self):
      ...
      with tf.variable_scope('sc_seq2seq'):
          ...
          self.trunc_norm_init = tf.initializers.truncated_normal(stddev=hps.trunc_norm_init_std.value)
          # self.trunc_norm_init = tf.truncated_normal_initializer(stddev=hps.trunc_norm_init_std)

          print("waterfriend 2")
          print(hps.emb_dim.value, type(hps.emb_dim.value))
          print(hps.trunc_norm_init_std.value, type(hps.trunc_norm_init_std.value))
          ...
          with tf.variable_scope('embedding'):
              ...
              decode_embedding = tf.get_variable('decode_embedding', [5, 10], dtype=tf.float32, initializer=self.trunc_norm_init) 
